...
export default function NavContactButton({ className = '', theme = 'light' }: NavContactButtonProps) {
  ...
  return (
    <Link
      href="/contact"
      className={`relative inline-flex items-center justify-center rounded-full font-medium font-['ABC_Diatype_Mono_Unlicensed_Trial'] leading-[72px] no-underline ${className}`}
      style={{
        ...
        paddingBottom: 'clamp(6px, 0.92vw, 12.615px)',
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: '1', width: 'clamp(60px, 7.33vw, 100px)', flexShrink: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Contact Us</div>
      <svg width="clamp(10px, 0.95vw, 13px)" height="clamp(11px, 1.03vw, 14px)" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ transition: 'fill 0.4s ease-in-out', flexShrink: 0 }}>
